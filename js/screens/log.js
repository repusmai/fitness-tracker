// ── Log Screen ────────────────────────────────────────────────────────────────
// Workout history and templates. QuickLog is launched from here but runs
// as a persistent overlay managed by app.js — not a screen takeover.

function Log({ workouts, exercises, templates, onQuickLog, onView, onDeleteTemplate, onEditTemplate, showInstall, onDismissInstall, isOnline, updateReady, onApplyUpdate, workoutActive }) {
    ...
    React.createElement('div', { ref: headerRef, style: { position: "absolute", top: 0, left: 0, right: 0, zIndex: 2, transform: headerHidden ? "translateY(-100%)" : "translateY(0)", transition: "transform 0.25s ease", willChange: "transform", background: "var(--bg)" } },
      React.createElement('div', { style: { padding: "24px 16px 0" } },
        React.createElement('div', { style: { display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 } },
          ...
          !workoutActive && React.createElement(Btn, { variant: "primary", onClick: onQuickLog, style: { padding: "10px 14px", borderRadius: 14, gap: 6 } },
            React.createElement(Icon, { name: "play", size: 15, color: "#fff" }), " Start"
          )
        ),
        ...
        React.createElement('div', { style: { padding: "12px 16px 16px" } },
          sorted.length === 0
            ? React.createElement('div', { style: { textAlign: "center", padding: "60px 20px" } },
                ...
                React.createElement('div', { style: { fontSize: 14, color: "var(--border2)" } }, "Tap Start to log your first session"))
          ...
          : templates.map(tpl => {
              ...
              return React.createElement('div', { key: tpl.id, style: { background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 16, overflow: "hidden" } },
                ...
                React.createElement('div', { style: { padding: "10px 14px 14px" } },
                  React.createElement(Btn, { variant: "primary", onClick: () => onQuickLog(tpl), style: { width: "100%", justifyContent: "center", padding: "11px", fontSize: 14 } },
                    React.createElement(Icon, { name: "play", size: 14, color: "#fff" }), " Start Workout"
                  )
                )
              );
            })
